  ...
  async updateNews(count)
  {
   this.setState({
            ...
            loading:false
                    ...
                    let data=  await fetch(`https://newsapi.org/v2/top-headlines?country=us&category=${this.props.Category}&apiKey=490c6cfe494c4c55ad322c03e63425e1&page=${count}&pagesize=9`);
    ...
    async componentDidMount(count) {
       
        this.updateNews(count);
    ...
    render() { 
        ...
        const handleclick=()=>{
            ...
            this.componentDidMount(this.state.count)
    ...
    return (
        ...
        {this.state.loading && this.state.data.map((elements)=>{ return(
...
<div className="d-grid gap-2 d-md-flex justify-content-md-end" id="button">
<button type="button" onClick={()=>{handleclick()}} className="btn btn-info">Next</button>
<button type="button" onClick={()=>{handleget()}} className="btn btn-info">Previous</button>
</div>
</>
